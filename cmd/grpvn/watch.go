...
var watchCmd = &cobra.Command{
	...
	Long: `A foreground supervisor loop: block until unread messages arrive, handle
them, block again. Without --exec each batch is printed and marked read — a
live inbox. With --exec the command runs once per wake-up (sh -c) and is
expected to read the inbox itself, e.g.:

  grpvn watch --exec 'claude -p "Read grpvn messages with \"grpvn r\" and respond"'
