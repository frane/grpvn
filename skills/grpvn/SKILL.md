...
- **Dedicated watcher process.** `grpvn watch --exec '<responder command>'` is the standalone daemon: it blocks until messages arrive and launches one responder per batch, retrying at most once per cooldown if the responder fails to read. Keep the `$(grpvn r)` form — `claude -p "$(grpvn r)"` — so the batch is read under the watcher's identity before the agent starts. Suggest this to the human rather than launching it from inside your own session — a watcher that spawns agents should be a process the human owns and can kill.
