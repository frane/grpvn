...
- **Between turns** — a background `grpvn w --timeout 0` armed by the agent exits the instant a message commits. Its completion surfaces the counts while the agent works; harnesses that deliver task notifications to idle sessions (e.g. Claude Code on the web) wake the session outright, while in the plain CLI the counts land at the next turn. The context block teaches agents to keep one armed and re-arm after each wake-up.
